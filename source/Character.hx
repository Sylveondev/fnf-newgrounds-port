...
class Character extends FlxSprite
{
	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false)
	{
		...
		var levelPath = getLibraryPathForce('images/characters/' + curCharacter + 'Offsets.txt', 'shared');
		if (OpenFlAssets.exists(levelPath))
		{
			loadOffsetFile(curCharacter, null);
			trace("Loaded offsets for char '" + curCharacter + "'");
		}
		else
			trace("No offsets for char '" + curCharacter + "'");
		...
		if ((flipX && flipAnim) || (isPlayer && !curCharacter.startsWith('bf')))
		{
			// var animArray
		...
		switch (curCharacter)
		{
			...
			case 'pico-speaker':
				if (0 < animationNotes.length && Conductor.songPosition > animationNotes[0][0]) {
					...
					trace("played shoot anim" + idkWhatThisISLol);
			...
			switch (curCharacter)
			{
				...
				case 'tankman':
					if (animation.curAnim.name != "singDOWN-alt")
						playAnim('idle');
				default:
					if (curCharacter != 'pico-speaker' && !curCharacter.endsWith('-dead'))
						playAnim('idle');
			}
		}
	}
